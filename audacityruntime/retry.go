...
// Matches spec §4: network errors are always retryable; server-derived errors
// carry a Retryable flag stamped at classification time (which already encodes
// "429 + BUDGET_EXCEEDED never retries").
func isRetryableError(err error) bool {
	if err == nil {
		return false
	}
	// Network / decode errors are always retryable.
...
// backoffDuration computes the jittered exponential backoff for a given attempt number
// (0-indexed after the first attempt), honouring any Retry-After header value.
//
